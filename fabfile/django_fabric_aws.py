...
def instance():
    """
    Creates an EC2 instance from an Ubuntu AMI and configures it as a
    Django server with nginx + gunicorn
    """
    ...
    print(_green("Waiting 60 seconds for server to initiate..."))
    time.sleep(60)
...
def update_packages():
    """
    Updates the python packages on the server as defined in
...
def _create_ec2_instance():
    ...
    while instance.state == u'pending':
        print(_yellow("Instance state: {}".format(instance.state)))
        time.sleep(10)
        instance.update()

    print(_green("Instance state: {}".format(instance.state)))
    print(_green("Public DNS: {}".format(instance.public_dns_name)))
...
def _apt(params):
    ...
    """
    for pkg in params:
        _sudo("apt-get install -qq {}".format(pkg))
